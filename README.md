...
Необходимо заранее на сервере {remoteServer} создать директорию {remoteServerDirPath}.
...
Архив содержит:
* Файл с информацией по процессам, info\_{SERVER}\_{timeNow}.txt.  
  Данная информация необходима для идентификации процессов.
  ~~~
...
Сервер с которого необходимо выполнить сбор диагностических данных памяти.  
...
Boolen значение, которое определяет, нужно ли выполнить снимок кучи (GC.heap_dump).
...
Boolen значение, которое определяет, нужно ли выполнить снимок потоков (Thread.print).
...
Boolen значение, которое определяет, нужно ли выполнить снимок классов (GC.class_histogram).
...
Boolen значение, которое определяет, нужно ли выполнить снимок утилизации памяти или диагностику утечки памяти (VM.native_memory).
* True - выполнить снимок утилизации памяти.  
Диагностика утечки памяти выполняется, если указан параметр DIAGNOSETIME.
* False - пропустить данный этап.
